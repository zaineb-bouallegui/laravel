...
<body class="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Style list</h6>
              </div>
              <a href="{{ url('/style/create') }}" class="btn btn-light btn-sm" title="Ajouter une piece ">
                            Add New
                        </a>
                ...
                <table class="table align-items-center justify-content-center mb-0">
                  <thead>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Numero</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">NameStyle</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Periode</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Artiste</th>
        ...
        <tr>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $style->id }}</p>
                      </td>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $style->nameStyle }}</p>
                      </td>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $style->periode }}</p>
                      </td>
                     
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $style->Artiste}}</p>
                      </td>
                     
                      <td>
                                            <a href="{{ url('/style/' . $style->id) }}" title="View Event"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                            <a href="{{ url('/style/' . $style->id .'/edit') }}" title="Edit Style"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>
  
                                            <form method="POST" action="{{ url('/style' . '/' . $style->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
