...
<body class="g-sidenav-show  bg-gray-200">
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Art list</h6>
              </div>
    ...
    <form method="GET" action="{{ route('art.index') }}" class="form-inline">
      <div class="input-group">
                ...
                <table class="table align-items-center justify-content-center mb-0">
                  <thead>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Numero</th>
                      ...
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder text-center opacity-7 ps-2">StyleId</th>
                      <th></th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        <tr>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $art->id }}</p>
                      </td>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $art->name }}</p>
                      </td>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $art->description }}</p>
                      </td>
                      <td>
                    <!-- Display the ID of the associated photo(s) -->
                    <p class="text-sm font-weight-bold mb-0">
                       <img class="avatar avatar-sm me-3 border-radius-lg" src="{{ asset('images/' . $art->image) }}" >
                    </p>
                </td>
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ $art->style_id }}</p>
                      </td>
                     
                      <td>
                                            <a href="{{ url('/art/' . $art->id) }}" title="View Event"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                            <a href="{{ url('/art/' . $art->id .'/edit') }}" title="Edit Art"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>
  
                                            <form method="POST" action="{{ url('/art' . '/' . $art->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
