                ...
                <div class="col-xl-3 col-lg-4 col-md-6">
                    @if ($location->photos->count() > 0)
                        <div class="gallery-item h-100">
                            <img src="{{ asset('storage/' . $location->photos[0]->url) }}" class="img-fluid" alt="">
                            <div class="gallery-links d-flex align-items-center justify-content-center">
                                <a href="{{ asset('storage/' . $location->photos[0]->url) }}" title="{{$location->name}} : {{$location->description}}" class="glightbox preview-link"><i class="bi bi-arrows-angle-expand"></i></a>
                                <a href="{{ route('location.details', ['id' => $location->id]) }}" class="details-link"><i class="bi bi-link-45deg"></i></a>
                            </div>
                        </div>
                    @endif
